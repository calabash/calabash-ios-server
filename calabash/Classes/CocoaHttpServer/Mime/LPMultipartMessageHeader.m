...
//-----------------------------------------------------------------
// implementation LPMutlipartMessageHeader
//-----------------------------------------------------------------
...
- (id) initWithData:(NSData *)data formEncoding:(NSStringEncoding) formEncoding {
	...
	while( offset < length - 2 ) {
		...
		if( (*(uint16_t*) (bytes+offset)  == fields_separator) && ((offset == length - 2) || !(isspace(bytes[offset+2])) )) {
			...
			LPMutlipartMessageHeaderField* field = [[LPMutlipartMessageHeaderField alloc] initWithData: fieldData  contentEncoding:formEncoding];
			if( field ) {
				[fields setObject:field forKey:field.name];
				//HTTPLogVerbose(@"LPMutlipartFormDataParser: Processed Header field '%@'",field.name);
			}
			else {
				//NSString* fieldStr = [[NSString  alloc] initWithData:fieldData encoding:NSASCIIStringEncoding];
				//HTTPLogWarn(@"LPMutlipartFormDataParser: Failed to parse MIME header field. Input ASCII string:%@",fieldStr);
			}
