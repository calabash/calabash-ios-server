...
/**
 * The LPHTTP server supports range requests in order to allow things like
 ...
 * That is, all data for the response has been returned to the LPHTTPConnection via the readDataOfLength method.
**/
- (BOOL)isDone;
...
/**
 * If you need time to calculate any part of the LPHTTP response headers (status code or header fields),
 ...
 * 
 * This method ties into the asynchronous response architecture of the LPHTTPConnection.
 * You should read the full discussion at the bottom of this header.
 * 
 * If you return YES from this method,
 * the LPHTTPConnection will wait for you to invoke the responseHasAvailableData method.
 * After you do, the LPHTTPConnection will again invoke this method to see if the response is ready to send the headers.
 * 
 ...
 * invoke any methods on the LPHTTPConnection after this method is called (as the connection may be deallocated).
**/
 ...
 * 
 * First the LPHTTPResponse is given the opportunity to postpone sending the LPHTTP response headers.
 ...
 * In situations such as this, the LPHTTPResponse simply implements the delayResponseHeaders method and returns YES.
 * After returning YES from this method, the LPHTTPConnection will wait until the response invokes its
 * responseHasAvailableData method. After this occurs, the LPHTTPConnection will again query the delayResponseHeaders
 ...
 * Asynchronously generating the body of the response is not an excuse to delay sending the headers.
 * 
 * After the response headers have been sent, the LPHTTPConnection calls your readDataOfLength method.
