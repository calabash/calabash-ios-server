...
@implementation LPHTTPDynamicFileResponse

- (id)initWithFilePath:(NSString *)fpath
         forConnection:(LPHTTPConnection *)parent
  ...
  while (offset < stopOffset) {
    ...
    if (memcmp(subBuffer, sep, sepLen) == 0) {
      if (!found1) {
        ...
        LPHTTPLogVerbose(@"%@[%p]: Found s1 at %lu", LP_THIS_FILE, self, (unsigned long) s1);
      }
      else {
        ...
        LPHTTPLogVerbose(@"%@[%p]: Found s2 at %lu", LP_THIS_FILE, self, (unsigned long) s2);
      }
      ...
      if (found1 && found2) {
        ...
        if (key) {
          ...
          if (value) {
            ...
            LPHTTPLogVerbose(@"%@[%p]: key(%@) -> value(%@)", LP_THIS_FILE, self, key, value);
