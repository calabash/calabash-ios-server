 ...
 *
 * LPHTTPConnection will invoke our readDataOfLength: method to fetch data.
...
@implementation LPHTTPAsyncFileResponse
...
- (void)pauseReadSource {
  if (!readSourceSuspended) {
    LPHTTPLogVerbose(@"%@[%p]: Suspending readSource", LP_THIS_FILE, self);
...
- (void)resumeReadSource {
  if (readSourceSuspended) {
    LPHTTPLogVerbose(@"%@[%p]: Resuming readSource", LP_THIS_FILE, self);
...
- (void)cancelReadSource {
  LPHTTPLogVerbose(@"%@[%p]: Canceling readSource", LP_THIS_FILE, self);
...
- (BOOL)openFileAndSetupReadSource {
  ...
  if (fileFD == NULL_FD) {
    LPHTTPLogError(@"%@: Unable to open file. filePath: %@", LP_THIS_FILE, filePath);
  ...
  dispatch_source_set_cancel_handler(readSource, ^{
    ...
    LPHTTPLogVerbose(@"%@: cancelBlock - Close fd[%i]", LP_THIS_FILE, theFileFD);
...
- (UInt64)contentLength {
  LPHTTPLogVerbose(@"%@[%p]: contentLength - %llu", LP_THIS_FILE, self, fileLength);
...
- (NSData *)readDataOfLength:(NSUInteger)length {
  ...
  if (data) {
    ...
    LPHTTPLogVerbose(@"%@[%p]: Returning data of length %lu", LP_THIS_FILE, self, (unsigned long) dataLength);
