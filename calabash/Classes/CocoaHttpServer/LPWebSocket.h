...
/**
 * The LPWebSocket class is thread-safe, generally via it's LPGCD queue.
 * All public API methods are thread-safe,
 * and the subclass API methods are thread-safe as they are all invoked on the same LPGCD queue.
**/
 ...
 * Public API
 * 
 * These methods are automatically called by the LPHTTPServer.
