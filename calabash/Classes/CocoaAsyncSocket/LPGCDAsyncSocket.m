...
NSString *const LPGCDAsyncSocketManuallyEvaluateTrust = @"LPGCDAsyncSocketManuallyEvaluateTrust";
#if TARGET_OS_IPHONE
NSString *const LPGCDAsyncSocketUseCFStreamForTLS = @"LPGCDAsyncSocketUseCFStreamForTLS";
#endif
NSString *const LPGCDAsyncSocketSSLPeerID = @"LPGCDAsyncSocketSSLPeerID";
...
NSString *const LPGCDAsyncSocketSSLSessionOptionFalseStart = @"LPGCDAsyncSocketSSLSessionOptionFalseStart";
NSString *const LPGCDAsyncSocketSSLSessionOptionSendOneByteRecord = @"LPGCDAsyncSocketSSLSessionOptionSendOneByteRecord";
...
/**
 * The LPGCDAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
 **/
@interface LPGCDAsyncReadPacket : NSObject {
@public
...
@implementation LPGCDAsyncReadPacket
...
/**
 * The LPGCDAsyncWritePacket encompasses the instructions for any given write.
 **/
@interface LPGCDAsyncWritePacket : NSObject {
@public
  NSData *buffer;
  NSUInteger bytesDone;
  long tag;
...
@implementation LPGCDAsyncWritePacket
...
/**
 * The LPGCDAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
 **/
@interface LPGCDAsyncSpecialPacket : NSObject {
@public
...
@implementation LPGCDAsyncSpecialPacket
...
@implementation LPGCDAsyncSocket {
  uint32_t flags;
  uint16_t config;
...
- (void)getDelegate:(id __autoreleasing*)delegatePtr
      delegateQueue:(dispatch_queue_t __autoreleasing*)delegateQueuePtr {
  if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey)) {
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError *__autoreleasing*)errPtr {
  ...
  int(^createSocket)(int, NSData *) = ^int(int domain, NSData *interfaceAddr) {
    ...
    if (status == -1) {
      ...
      LPGCDLogVerbose(@"close(socketFD)");
      close(socketFD);
      return SOCKET_NULL;
    }
  ...
  dispatch_block_t block = ^{
    @autoreleasepool {
      ...
      if (enableIPv4) {
        LPGCDLogVerbose(@"Creating IPv4 socket");
      ...
      if (enableIPv6) {
        LPGCDLogVerbose(@"Creating IPv6 socket");
        ...
        if (self->socket6FD == SOCKET_NULL) {
          if (self->socket4FD != SOCKET_NULL) {
            LPGCDLogVerbose(@"close(socket4FD)");
      ...
      if (enableIPv4) {
        ...
        __weak LPGCDAsyncSocket *weakSelf = self;

        dispatch_source_set_event_handler(self->accept4Source, ^{
          @autoreleasepool {
#pragma clang diagnostic push
#pragma clang diagnostic warning "-Wimplicit-retain-self"

            __strong LPGCDAsyncSocket *strongSelf = weakSelf;
            if (strongSelf == nil) return_from_block;

            LPGCDLogVerbose(@"event4Block");

            unsigned long i = 0;
            unsigned long numPendingConnections = dispatch_source_get_data(acceptSource);

            LPGCDLogVerbose(@"numPendingConnections: %lu", numPendingConnections);
...
#if !OS_OBJECT_USE_OBJC
          LPGCDLogVerbose(@"dispatch_release(accept4Source)");
...
#pragma clang diagnostic pop
        ...
        LPGCDLogVerbose(@"dispatch_resume(accept4Source)");
      ...
      if (enableIPv6) {
        ...
        dispatch_source_set_event_handler(self->accept6Source, ^{
          @autoreleasepool {
#pragma clang diagnostic push
#pragma clang diagnostic warning "-Wimplicit-retain-self"
...
#if !OS_OBJECT_USE_OBJC
          LPGCDLogVerbose(@"dispatch_release(accept6Source)");
          dispatch_release(acceptSource);
#endif

          LPGCDLogVerbose(@"close(socket6FD)");
          close(socketFD);

#pragma clang diagnostic pop
        });

        LPGCDLogVerbose(@"dispatch_resume(accept6Source)");
  ...
  if (result == NO) {
    LPGCDLogInfo(@"Error in accept: %@", err);
...
- (BOOL)doAccept:(int)parentSocketFD {
  ...
  if (parentSocketFD == socket4FD) {
    ...
    if (childSocketFD == -1) {
      LPGCDLogWarn(@"Accept failed with error: %@", [self errnoError]);
      return NO;
    }
  ...
  if (result == -1) {
    LPGCDLogWarn(@"Error enabling non-blocking IO on accepted socket (fcntl)");
    return NO;
  }
...
- (BOOL)connectToHost:(NSString *)inHost
  ...
  dispatch_block_t block = ^{
    @autoreleasepool {
      ...
      LPGCDLogVerbose(@"Dispatching DNS lookup...");
...
#pragma clang diagnostic warning "-Wimplicit-retain-self"
          ...
          NSMutableArray *addresses = [LPGCDAsyncSocket lookupHost:hostCpy port:port error:&lookupErr];
          ...
          if (lookupErr) {
            ...
            for (NSData *address in addresses) {
              if (!address4 && [LPGCDAsyncSocket isIPv4Address:address]) {
                address4 = address;
              }
              else if (!address6 && [LPGCDAsyncSocket isIPv6Address:address]) {
                address6 = address;
              }
            }
...
- (void)lookup:(int)aStateIndex didSucceedWithAddress4:(NSData *)address4 address6:(NSData *)address6 {
  ...
  if (aStateIndex != stateIndex) {
    LPGCDLogInfo(@"Ignoring lookupDidSucceed, already disconnected");
...
- (void)lookup:(int)aStateIndex didFail:(NSError *)error {
  ...
  if (aStateIndex != stateIndex) {
    LPGCDLogInfo(@"Ignoring lookup:didFail: - already disconnected");
...
- (BOOL)connectWithAddress4:(NSData *)address4 address6:(NSData *)address6 error:(NSError *__autoreleasing*)errPtr {
  ...
  LPGCDLogVerbose(@"IPv4: %@:%hu", [[self class] hostFromAddress:address4], [[self class] portFromAddress:address4]);
  LPGCDLogVerbose(@"IPv6: %@:%hu", [[self class] hostFromAddress:address6], [[self class] portFromAddress:address6]);
  ...
  if (connectInterface) {
    LPGCDLogVerbose(@"Binding socket...");
...
- (void)didConnect:(int)aStateIndex {
  ...
  if (aStateIndex != stateIndex) {
    LPGCDLogInfo(@"Ignoring didConnect, already disconnected");
...
- (void)didNotConnect:(int)aStateIndex error:(NSError *)error {
  ...
  if (aStateIndex != stateIndex) {
    LPGCDLogInfo(@"Ignoring didNotConnect, already disconnected");
...
#pragma clang diagnostic warning "-Wimplicit-retain-self"
      ...
      LPGCDLogVerbose(@"dispatch_release(connectTimer)");
...
- (void)closeWithError:(NSError *)error {
  ...
  if (!accept4Source && !accept6Source && !readSource && !writeSource) {
    LPGCDLogVerbose(@"manually closing close");
    ...
    if (accept4Source) {
      LPGCDLogVerbose(@"dispatch_source_cancel(accept4Source)");
    ...
    if (accept6Source) {
      LPGCDLogVerbose(@"dispatch_source_cancel(accept6Source)");
    ...
    if (readSource) {
      LPGCDLogVerbose(@"dispatch_source_cancel(readSource)");
    ...
    if (writeSource) {
      LPGCDLogVerbose(@"dispatch_source_cancel(writeSource)");
...
- (NSError *)badConfigError:(NSString *)errMsg {
  ...
  return [NSError errorWithDomain:LPGCDAsyncSocketErrorDomain code:LPGCDAsyncSocketBadConfigError userInfo:userInfo];
}

- (NSError *)badParamError:(NSString *)errMsg {
  NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];

  return [NSError errorWithDomain:LPGCDAsyncSocketErrorDomain code:LPGCDAsyncSocketBadParamError userInfo:userInfo];
}
...
- (NSError *)connectTimeoutError {
  NSString *errMsg = NSLocalizedStringWithDefaultValue(@"LPGCDAsyncSocketConnectTimeoutError",
                                                       @"LPGCDAsyncSocket", [NSBundle mainBundle],
                                                       @"Attempt to connect to host timed out", nil);

  NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];

  return [NSError errorWithDomain:LPGCDAsyncSocketErrorDomain code:LPGCDAsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
- (NSError *)readMaxedOutError {
  NSString *errMsg = NSLocalizedStringWithDefaultValue(@"LPGCDAsyncSocketReadMaxedOutError",
                                                       @"LPGCDAsyncSocket", [NSBundle mainBundle],
                                                       @"Read operation reached set maximum length", nil);

  NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];

  return [NSError errorWithDomain:LPGCDAsyncSocketErrorDomain code:LPGCDAsyncSocketReadMaxedOutError userInfo:info];
}
...
- (NSError *)readTimeoutError {
  NSString *errMsg = NSLocalizedStringWithDefaultValue(@"LPGCDAsyncSocketReadTimeoutError",
                                                       @"LPGCDAsyncSocket", [NSBundle mainBundle],
                                                       @"Read operation timed out", nil);

  NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];

  return [NSError errorWithDomain:LPGCDAsyncSocketErrorDomain code:LPGCDAsyncSocketReadTimeoutError userInfo:userInfo];
}
...
- (NSError *)writeTimeoutError {
  NSString *errMsg = NSLocalizedStringWithDefaultValue(@"LPGCDAsyncSocketWriteTimeoutError",
                                                       @"LPGCDAsyncSocket", [NSBundle mainBundle],
                                                       @"Write operation timed out", nil);

  NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];

  return [NSError errorWithDomain:LPGCDAsyncSocketErrorDomain code:LPGCDAsyncSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError {
  NSString *errMsg = NSLocalizedStringWithDefaultValue(@"LPGCDAsyncSocketClosedError",
                                                       @"LPGCDAsyncSocket", [NSBundle mainBundle],
                                                       @"Socket closed by remote peer", nil);

  NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];

  return [NSError errorWithDomain:LPGCDAsyncSocketErrorDomain code:LPGCDAsyncSocketClosedError userInfo:userInfo];
}

- (NSError *)otherError:(NSString *)errMsg {
  NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];

  return [NSError errorWithDomain:LPGCDAsyncSocketErrorDomain code:LPGCDAsyncSocketOtherError userInfo:userInfo];
}
...
#pragma clang diagnostic warning "-Wimplicit-retain-self"
      ...
      LPGCDLogVerbose(@"readEventBlock");

      strongSelf->socketFDBytesAvailable = dispatch_source_get_data(strongSelf->readSource);
      LPGCDLogVerbose(@"socketFDBytesAvailable: %lu", strongSelf->socketFDBytesAvailable);
...
#pragma clang diagnostic warning "-Wimplicit-retain-self"
      ...
      LPGCDLogVerbose(@"writeEventBlock");
...
#pragma clang diagnostic warning "-Wimplicit-retain-self"
    ...
    LPGCDLogVerbose(@"readCancelBlock");

#if !OS_OBJECT_USE_OBJC
    LPGCDLogVerbose(@"dispatch_release(readSource)");
...
#pragma clang diagnostic warning "-Wimplicit-retain-self"
    ...
    LPGCDLogVerbose(@"writeCancelBlock");

#if !OS_OBJECT_USE_OBJC
    LPGCDLogVerbose(@"dispatch_release(writeSource)");
...
#pragma clang diagnostic pop
  ...
  LPGCDLogVerbose(@"dispatch_resume(readSource)");
...
- (BOOL)usingCFStreamForTLS {
  ...
  if ((flags & kSocketSecure) && (flags & kUsingCFStreamForTLS)) {
    // The startTLS method was given the LPGCDAsyncSocketUseCFStreamForTLS flag.
...
- (void)suspendReadSource {
  if (!(flags & kReadSourceSuspended)) {
    LPGCDLogVerbose(@"dispatch_suspend(readSource)");
...
- (void)suspendWriteSource {
  if (!(flags & kWriteSourceSuspended)) {
    LPGCDLogVerbose(@"dispatch_suspend(writeSource)");
...
- (void)resumeWriteSource {
  if (flags & kWriteSourceSuspended) {
    LPGCDLogVerbose(@"dispatch_resume(writeSource)");
...
- (void)readDataWithTimeout:(NSTimeInterval)timeout
  ...
  if (offset > [buffer length]) {
    LPGCDLogWarn(@"Cannot read: offset > [buffer length]");
    return;
  }
...
- (void)readDataToLength:(NSUInteger)length
            ...
            bufferOffset:(NSUInteger)offset
                     tag:(long)tag {
  if (length == 0) {
    LPGCDLogWarn(@"Cannot read: length == 0");
    return;
  }
  if (offset > [buffer length]) {
    LPGCDLogWarn(@"Cannot read: offset > [buffer length]");
...
- (void)readDataToData:(NSData *)data
          ...
          bufferOffset:(NSUInteger)offset
             maxLength:(NSUInteger)maxLength
                   tag:(long)tag {
  if ([data length] == 0) {
    LPGCDLogWarn(@"Cannot read: [data length] == 0");
    return;
  }
  if (offset > [buffer length]) {
    LPGCDLogWarn(@"Cannot read: offset > [buffer length]");
  ...
  if (maxLength > 0 && maxLength < [data length]) {
    LPGCDLogWarn(@"Cannot read: maxLength > 0 && maxLength < [data length]");
    return;
  }
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr {
  ...
  dispatch_block_t block = ^{
    ...
    if (!self->currentRead || ![self->currentRead isKindOfClass:[LPGCDAsyncReadPacket class]]) {
  ...
  if ((currentRead == nil) && (flags & kConnected)) {
    if ([readQueue count] > 0) {
      ...
      if ([currentRead isKindOfClass:[LPGCDAsyncSpecialPacket class]]) {
        LPGCDLogVerbose(@"Dequeued LPGCDAsyncSpecialPacket");
        ...
        LPGCDLogVerbose(@"Dequeued LPGCDAsyncReadPacket");
...
- (void)doReadData {
  ...
  if ((currentRead == nil) || (flags & kReadsPaused)) {
    LPGCDLogVerbose(@"No currentRead or kReadsPaused");
  ...
  if ([self usingCFStreamForTLS]) {
    ...
    // Requested CFStream, rather than SecureTransport, for TLS (via LPGCDAsyncSocketUseCFStreamForTLS)
  ...
  if ((hasBytesAvailable == NO) && ([preBuffer availableBytes] == 0)) {
    LPGCDLogVerbose(@"No data available to read...");
  ...
  if (flags & kStartingReadTLS) {
    LPGCDLogVerbose(@"Waiting for SSL/TLS handshake to complete");
  ...
  if ([preBuffer availableBytes] > 0) {
    ...
    LPGCDLogVerbose(@"copied(%lu) preBufferLength(%zu)", (unsigned long) bytesToCopy, [preBuffer availableBytes]);
  ...
  if (!done && !error && !socketEOF && hasBytesAvailable) {
    ...
    if (flags & kSocketSecure) {
      if ([self usingCFStreamForTLS]) {
        ...
        LPGCDLogVerbose(@"CFReadStreamRead(): result = %i", (int) result);
        ...
        do {
          ...
          LPGCDLogVerbose(@"read from secure socket = %u", (unsigned) loop_bytesRead);
      ...
      LPGCDLogVerbose(@"read from socket = %i", (int) result);
    ...
    if (bytesRead > 0) {
      ...
      if (currentRead->readLength > 0) {
        ...
        if (readIntoPreBuffer) {
          ...
          [preBuffer didWrite:bytesRead];
          LPGCDLogVerbose(@"read data into preBuffer - preBuffer.length = %zu", [preBuffer availableBytes]);

          // Search for the terminating sequence

          NSUInteger bytesToCopy = [currentRead readLengthForTermWithPreBuffer:preBuffer found:&done];
          LPGCDLogVerbose(@"copying %lu bytes from preBuffer", (unsigned long) bytesToCopy);
          ...
          // Remove the copied bytes from the prebuffer
          [preBuffer didRead:bytesToCopy];
          LPGCDLogVerbose(@"preBuffer.length = %zu", [preBuffer availableBytes]);
          ...
          if (overflow == 0) {
            ...
            LPGCDLogVerbose(@"copying %ld overflow bytes into preBuffer", (long) overflow);
...
- (void)doReadEOF {
  ...
  if ((flags & kStartingReadTLS) || (flags & kStartingWriteTLS)) {
    ...
    LPGCDLogVerbose(@"Socket reached EOF, but there is still data available in prebuffer");
...
#pragma clang diagnostic warning "-Wimplicit-retain-self"
      ...
      LPGCDLogVerbose(@"dispatch_release(readTimer)");
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr {
  ...
  dispatch_block_t block = ^{
    ...
    if (!self->currentWrite || ![self->currentWrite isKindOfClass:[LPGCDAsyncWritePacket class]]) {
  ...
  if ((currentWrite == nil) && (flags & kConnected)) {
    if ([writeQueue count] > 0) {
      ...
      if ([currentWrite isKindOfClass:[LPGCDAsyncSpecialPacket class]]) {
        ...
        LPGCDLogVerbose(@"Dequeued LPGCDAsyncWritePacket");
...
- (void)doWriteData {
  ...
  if ((currentWrite == nil) || (flags & kWritesPaused)) {
    LPGCDLogVerbose(@"No currentWrite or kWritesPaused");
  ...
  if (!(flags & kSocketCanAcceptBytes)) {
    LPGCDLogVerbose(@"No space available to write...");
  ...
  if (flags & kSocketSecure) {
    if ([self usingCFStreamForTLS]) {
      ...
      LPGCDLogVerbose(@"CFWriteStreamWrite(%lu) = %li", (unsigned long) bytesToWrite, result);
    ...
    LPGCDLogVerbose(@"wrote to socket = %zd", result);
  ...
  if (bytesWritten > 0) {
    ...
    LPGCDLogVerbose(@"currentWrite->bytesDone = %lu", (unsigned long) currentWrite->bytesDone);
...
#pragma clang diagnostic warning "-Wimplicit-retain-self"
      ...
      LPGCDLogVerbose(@"dispatch_release(writeTimer)");
...
- (void)ssl_startTLS {
  ...
  LPGCDLogVerbose(@"Starting TLS (via SecureTransport)...");
  ...
  BOOL shouldManuallyEvaluateTrust = [[tlsSettings objectForKey:LPGCDAsyncSocketManuallyEvaluateTrust] boolValue];
  ...
  //  3. LPGCDAsyncSocketSSLPeerID
  //  4. LPGCDAsyncSocketSSLProtocolVersionMin
  //  5. LPGCDAsyncSocketSSLProtocolVersionMax
  //  6. LPGCDAsyncSocketSSLSessionOptionFalseStart
  //  7. LPGCDAsyncSocketSSLSessionOptionSendOneByteRecord
  ...
  value = [tlsSettings objectForKey:LPGCDAsyncSocketSSLPeerID];
  if ([value isKindOfClass:[NSData class]]) {
    ...
    NSAssert(NO, @"Invalid value for LPGCDAsyncSocketSSLPeerID. Value must be of type NSData."
             @" (You can convert strings to data using a method like"
             @" [string dataUsingEncoding:NSUTF8StringEncoding])");

    [self closeWithError:[self otherError:@"Invalid value for LPGCDAsyncSocketSSLPeerID."]];
    return;
  }

  // 4. LPGCDAsyncSocketSSLProtocolVersionMin

  value = [tlsSettings objectForKey:LPGCDAsyncSocketSSLProtocolVersionMin];
  if ([value isKindOfClass:[NSNumber class]]) {
    ...
    NSAssert(NO, @"Invalid value for LPGCDAsyncSocketSSLProtocolVersionMin. Value must be of type NSNumber.");

    [self closeWithError:[self otherError:@"Invalid value for LPGCDAsyncSocketSSLProtocolVersionMin."]];
    return;
  }

  // 5. LPGCDAsyncSocketSSLProtocolVersionMax

  value = [tlsSettings objectForKey:LPGCDAsyncSocketSSLProtocolVersionMax];
  if ([value isKindOfClass:[NSNumber class]]) {
    ...
    NSAssert(NO, @"Invalid value for LPGCDAsyncSocketSSLProtocolVersionMax. Value must be of type NSNumber.");

    [self closeWithError:[self otherError:@"Invalid value for LPGCDAsyncSocketSSLProtocolVersionMax."]];
    return;
  }

  // 6. LPGCDAsyncSocketSSLSessionOptionFalseStart

  value = [tlsSettings objectForKey:LPGCDAsyncSocketSSLSessionOptionFalseStart];
  if ([value isKindOfClass:[NSNumber class]]) {
    ...
    NSAssert(NO, @"Invalid value for LPGCDAsyncSocketSSLSessionOptionFalseStart. Value must be of type NSNumber.");

    [self closeWithError:[self otherError:@"Invalid value for LPGCDAsyncSocketSSLSessionOptionFalseStart."]];
    return;
  }

  // 7. LPGCDAsyncSocketSSLSessionOptionSendOneByteRecord

  value = [tlsSettings objectForKey:LPGCDAsyncSocketSSLSessionOptionSendOneByteRecord];
  if ([value isKindOfClass:[NSNumber class]]) {
    ...
    NSAssert(NO, @"Invalid value for LPGCDAsyncSocketSSLSessionOptionSendOneByteRecord."
             @" Value must be of type NSNumber.");

    [self closeWithError:[self otherError:@"Invalid value for LPGCDAsyncSocketSSLSessionOptionSendOneByteRecord."]];
    return;
  }
  ...
  if ([value isKindOfClass:[NSArray class]]) {
    ...
    NSAssert(NO, @"Invalid value for LPGCDAsyncSocketSSLCipherSuites. Value must be of type NSArray.");

    [self closeWithError:[self otherError:@"Invalid value for LPGCDAsyncSocketSSLCipherSuites."]];
    return;
  }
  ...
  {
    NSAssert(NO, @"Invalid value for LPGCDAsyncSocketSSLDiffieHellmanParameters. Value must be of type NSData.");

    [self closeWithError:[self otherError:@"Invalid value for LPGCDAsyncSocketSSLDiffieHellmanParameters."]];
    return;
  }
#endif
...
#pragma clang diagnostic pop
  if (value) {
    NSAssert(NO, @"Security option unavailable - kCFStreamSSLLevel"
             @" - You must use LPGCDAsyncSocketSSLProtocolVersionMin & LPGCDAsyncSocketSSLProtocolVersionMax");
...
- (void)ssl_continueSSLHandshake {
  ...
  if (status == noErr) {
    LPGCDLogVerbose(@"SSLHandshake complete");
    ...
    LPGCDLogVerbose(@"SSLHandshake peerAuthCompleted - awaiting delegate approval");
...
#pragma clang diagnostic pop
    ...
    if (delegateQueue && [theDelegate respondsToSelector:@selector(socket:didReceiveTrust:completionHandler:)]) {
      ...
      NSString *msg = @"LPGCDAsyncSocketManuallyEvaluateTrust specified in tlsSettings,"
    ...
    LPGCDLogVerbose(@"SSLHandshake continues...");
...
- (void)ssl_shouldTrustPeer:(BOOL)shouldTrust stateIndex:(int)aStateIndex {
  ...
  if (aStateIndex != stateIndex) {
    LPGCDLogInfo(@"Ignoring ssl_shouldTrustPeer - invalid state (maybe disconnected)");
...
- (void)cf_startTLS {
  ...
  LPGCDLogVerbose(@"Starting TLS (via CFStream)...");
  ...
  LPGCDLogVerbose(@"Waiting for SSL Handshake to complete...");
}
...
+ (void)startCFStreamThreadIfNeeded {
  ...
  dispatch_once(&predicate, ^{
    ...
    cfstreamThreadSetupQueue = dispatch_queue_create("LPGCDAsyncSocket-CFStreamThreadSetup", DISPATCH_QUEUE_SERIAL);
  });
...
#pragma clang diagnostic warning "-Wimplicit-retain-self"
      ...
      if (cfstreamThreadRetainCount == 0) {
        LPGCDLogWarn(@"Logic error concerning cfstreamThread start / stop");
        return_from_block;
      }
      ...
      if (--cfstreamThreadRetainCount == 0) {
        ...
        // wake up the thread
        [LPGCDAsyncSocket performSelector:@selector(ignore:)
...
+ (void)cfstreamThread {
  @autoreleasepool {
    ...
    LPGCDLogInfo(@"CFStreamThread: Started");
    ...
    LPGCDLogInfo(@"CFStreamThread: Stopped");
  }
}
...
- (BOOL)createReadAndWriteStream {
  ...
  LPGCDLogVerbose(@"Creating read and write stream...");
  ...
  if ((readStream == NULL) || (writeStream == NULL)) {
    LPGCDLogWarn(@"Unable to create read and write stream...");
...
- (BOOL)addStreamsToRunLoop {
  ...
  if (!(flags & kAddedStreamsToRunLoop)) {
    LPGCDLogVerbose(@"Adding streams to runloop...");
...
- (void)removeStreamsFromRunLoop {
  ...
  if (flags & kAddedStreamsToRunLoop) {
    LPGCDLogVerbose(@"Removing streams from runloop...");
...
- (BOOL)openStreams {
  ...
  if ((readStatus == kCFStreamStatusNotOpen) || (writeStatus == kCFStreamStatusNotOpen)) {
    LPGCDLogVerbose(@"Opening read and write stream...");

    BOOL r1 = CFReadStreamOpen(readStream);
    BOOL r2 = CFWriteStreamOpen(writeStream);

    if (!r1 || !r2) {
      LPGCDLogError(@"Error in CFStreamOpen");
      return NO;
    }
  }
...
+ (BOOL)getHost:(NSString *__autoreleasing*)hostPtr
