...
@implementation LPViewJSONSerializer
...
+ (NSObject *) jsonify: (id<NSObject>) obj {
	...
	if([obj isKindOfClass: NSArray.class] || [obj isKindOfClass: NSSet.class]) {
		...
		for(id subObject in theArray) {
			id subJson = [LPViewJSONSerializer jsonify: subObject];
	...
	if([obj isKindOfClass: NSDictionary.class]) {
		...
		for(id key in [theDictionary allKeys]) {
			...
			id subJson = [LPViewJSONSerializer jsonify: value];
			if(subJson != nil) {
	...
	if( [obj isKindOfClass: NSValue.class] ) {
		return  [LPViewJSONSerializer extractInstanceFromValue: (NSValue *) obj];
	}
	
#if TARGET_OS_IPHONE
	if ([obj isKindOfClass: UIColor.class]) {
		return [LPViewJSONSerializer extractInstanceFromColor: (UIColor *) obj];
	}
#else
