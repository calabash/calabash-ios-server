...
- (NSDictionary *) serializeView: (FrankSerializeViewType *) view {
	...
	if(accessibilityIdentifier.length == 0) {
		accessibilityIdentifier = view.accessibilityLabel;
	}
...
- (id) valueForAttribute: (NSString *) attribute onObject: (NSObject *) object {
	...
	@catch (NSException *exception) {
		NSLog(@"Warning \"%@\" does not have a value for the key \"%@\".", object.class, attribute);
	}
	@finally {
		...
		if (!value) {
			...
			if(![object respondsToSelector: selector]) {
				NSLog(@"Warning \"%@\" does not respond to \"%@\".", object.class, attribute);
    ...
    if( [value isKindOfClass: UIColor.class]) { 
        value = [LPViewJSONSerializer extractInstanceFromColor: (UIColor *) value];
    }
    
    if([value isKindOfClass: UIFont.class]) {
        value = [LPViewJSONSerializer extractInstanceFromFont: (UIFont *) value];
    }
#else
    ...
    if([value isKindOfClass: NSValue.class]) {
        value = [LPViewJSONSerializer extractInstanceFromValue: (NSValue *) value];
    }
