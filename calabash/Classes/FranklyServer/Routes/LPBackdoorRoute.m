...
- (NSDictionary *) JSONResponseForMethod:(NSString *) method URI:(NSString *) path data:(NSDictionary *) data {
  ...
  if ([[[UIApplication sharedApplication] delegate] respondsToSelector:sel]) {
    ...
    return [NSDictionary dictionaryWithObjectsAndKeys:res, @"result", @"SUCCESS", @"outcome", nil];
  } else {

    NSString *details = [NSString stringWithFormat:@"you must define the selector '%@' in your UIApplicationDelegate.", selStr];
    ...
    NSString *reasonStr = [NSString stringWithFormat:@"application delegate does not respond to selector '%@'", selStr];
    return [NSDictionary dictionaryWithObjectsAndKeys:detailsStr, @"details", reasonStr, @"reason", @"FAILURE", @"outcome", nil];
  }
}
