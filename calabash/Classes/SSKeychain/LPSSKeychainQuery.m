...
@implementation LPSSKeychainQuery
...
- (id) init {
  ...
  if (self) {
    ...
    if (ss_ios_version_lt(@"7.0")) {
      _synchronizationMode = LPSSKeychainQuerySynchronizationModeNotAvailable;
    }
#else
    no solution implemented for MacOS
#endif
  }
  return self;
}

- (void) setSynchronizationMode:(LPSSKeychainQuerySynchronizationMode) synchronizationMode {
...
- (BOOL)save:(NSError *__autoreleasing *)error {
  OSStatus status = LPSSKeychainErrorBadArguments;
  ...
  if (self.label) {
	...
	CFTypeRef accessibilityType = [LPSSKeychain accessibilityType];
...
- (NSMutableDictionary *)query {
  ...
  switch (self.synchronizationMode) {
    case LPSSKeychainQuerySynchronizationModeNo: {
      value = @NO;
      break;
    }
    case LPSSKeychainQuerySynchronizationModeYes: {
      value = @YES;
      break;
    }
    case LPSSKeychainQuerySynchronizationModeAny: {
      value = (__bridge id)(kSecAttrSynchronizableAny);
      break;
    }
    case LPSSKeychainQuerySynchronizationModeNotAvailable: {
      value = nil;
      break;
    }
  }
...
+ (NSError *)errorWithCode:(OSStatus) code {
  ...
  switch (code) {
    ...
    case LPSSKeychainErrorBadArguments: message = NSLocalizedStringFromTable(@"SSKeychainErrorBadArguments", @"SSKeychain", nil); break;
  ...
  return [NSError errorWithDomain:kLPSSKeychainErrorDomain code:code userInfo:userInfo];
}
